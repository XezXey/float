...
class InferenceAgent:
	...
	def run_inference(
		...
		end_inf = time.time()
		print(f"[cyan]> [#FLOAT] Inference completed () in [bold]{end_inf - start_inf:.2f}[/bold] seconds.[/cyan]")
		print(f"[cyan]> [#FLOAT] Inference FPS = [bold]{d_hat.shape[0] / (end_inf - start_inf):.2f}[/bold] frames/sec.[/cyan]")

		start_save = time.time()
		res_video_path = self.save_video(d_hat, res_video_path, audio_path)
		end_save = time.time()
		print(f"[cyan]> [#FLOAT] Video saving completed in [bold]{end_save - start_save:.2f}[/bold] seconds.[/cyan]")
		if verbose: print(f"[green]> [Done] result saved at {res_video_path}[/green]")
...
if __name__ == '__main__':
	...
	print(f"[bold green]> [#FLOAT] Total execution (Preprocess + FLOAT + Save) time: {end - start:.2f} seconds.[/bold green]")
	print(f"[bold green]> [#FLOAT] Total execution FPS = {misc['n_frames'] / (end - start):.2f} frames/sec.[/bold green]")
