...
class InferenceAgent:
	...
	def load_model(self) -> None:
		self.G = FLOAT(self.opt, onnx_model_path=self.opt.onnx_model_path, onnx_provider=self.opt.onnx_provider)
	...
	def run_inference(
		...
		end_inf = time.time()
		print(f"> [#ONNXRUNTIME] Inference completed () in {end_inf - start_inf:.2f} seconds.")
		print(f"> [#ONNXRUNTIME] Inference FPS = {d_hat.shape[0] / (end_inf - start_inf):.2f} frames/sec.")

		start_save = time.time()
		res_video_path = self.save_video(d_hat, res_video_path, audio_path)
		end_save = time.time()
		print(f"> [#ONNXRUNTIME] Video saving completed in {end_save - start_save:.2f} seconds.")
...
class InferenceOptions(BaseOptions):
	...
	def initialize(self, parser):
		...
		parser.add_argument("--onnx_model_path",
				required=True, type=str, help="Path to the ONNX model file exported by export_onnx.py")
		parser.add_argument("--onnx_provider",
				default="cuda", type=str, help="Execution provider target for ONNX Runtime: 'cuda' or 'cpu'")
...
if __name__ == '__main__':
	...
	print(f"> [#ONNXRUNTIME] Total execution (Preprocess + ONNXRUNTIME + Save) time: {end - start:.2f} seconds.")